...
class TextDataSchem
{
    ...
    protected $schemItems = [
        ...
        'sys' => ['sys', [], 1, 'Информация о создании/редактировании', true, false, 'list', '', [], [], []],
        'inc' => ['inc', [], 2, 'Инкремент для базы', true, false, 'list', '', [], [], []],
        'name' => ['name', [], 3, 'Имя колонки', false, false, 'list', '', [], [], []],
        ...
        'fieldInfo' => ['fieldInfo', [], 8, 'Настройки поля для формы', false, false, 'list', '', [], [], []],
        'linkProps' => ['linkProps', [], 9, 'Настройки для связи', false, false, 'list', '', [], [], []],
        'listItems' => ['listItems', [], 10, 'Текущие элементы для выбора', false, false, 'list', '', [], [], []],
    ]; 
    ...
    */
    ...
    /* end of Преобразование значений для отображения */
    ...
    */
    ...
    {
        $result = [];
        $schemFields = array_column($this->getSchem(), 3);
        $schemNames = array_column($this->getSchem(), 2);

        foreach ($info as $infoId => $value) {
            if (!in_array($infoId, $schemFields) and !in_array($infoId, $schemNames) and $value != '') {
