...
class FormHelpers
{
    ...
    */
    ...
    public function setFieldsFromSchem($schemItems, $info=[])
    {
        //echo 'Формируем поля из схемы';
        //print_r($schemItems);
        
        foreach ($schemItems as $sId => $sInfo){
            $itemId = $sInfo[3];
            $itemValue = $info[$itemId] ?? '';
            $this->fields[$sId] = $this->setFieldFromSchem ($sInfo, $itemValue);

            //echo '<hr>'.$sId.'<br>';
            //print_r($sInfo);
            //echo '<br>'.$sInfo[6].'<br>';
            //print_r($this->fields[$itemId]);
        }
    }

    protected function setFieldFromSchem ($schem, $value)
    {

        $field = [];

        $fieldType = (isset($schem[8]) and $schem[8] != '')? $schem[8] : 1;

        $FieldInfo = $this->fieldTypes[$fieldType];
        $primType = $FieldInfo[1] ?? 'input';
        $subType = $FieldInfo[2] ?? '';
        //$dataType = $info[3];

        $label = $schem[10] ?? $schem[2];
        $description = (isset($schem[12]) and $schem[12] != '')? $schem[12] : "";
        $icon = (isset($FieldInfo[4]) and $FieldInfo[4] != '')? '<i class="'.$FieldInfo[4].' icon"></i> ' : '';

        $fieldId = $schem[3];
        $fieldName = 'form_info['.$fieldId.']';
        $fieldPlaceholder = $schem[11] ?? "";
        $display = (isset($schem[9]) and $schem[9] != '')? $this->fieldViewTypes[$schem[9]][0] : 'normal';        

        //$listValues = [1=>["Элемент 1"], 2=>["Элемент 2"], 3=>["Элемент 3"],];
        $listValues = [];
        
        if ($schem[6] == 'link' or $schem[6] == 'list' or $schem[6] == 'file'){
            $listValues = $schem[17];
        ...
        return [$primType, $subType, $wraperProps, $fieldProps, $FieldInfo];
    }
    ...
    public function setFieldsOrder($order)
    {
        ...
        foreach ($order as $sId){
            $newFields[$sId] = $this->fields[$sId];
        }

        $this->fields = $newFields;
    }

    public function hideFields($order)
    {        
        foreach ($order as $sId){
            $this->fields[$sId][3]['display'] = 'hidden';
            $this->fields[$sId][3]['listValues'] = [];
        }
    }
    ...
    {        
        ...
        $html = '
        <h3>'.$title.'</h3>
        <form class="ui form" action="'.$url.'" method="post" id="schemForm">
    ...
    protected function viewFields ($html='')
    {
        foreach ($this->fields as $fieldId => $fieldInfo){
            $html .= $this->viewField ($fieldInfo);
        }
        return $html;
    }
    ...
    protected function viewField ($fieldInfo)
    {
        //$html = $this->genFieldWrapper ($primType, $subType, $wraperProps, $fieldProps, $info);
        $html = $this->genFieldWrapper ($fieldInfo[0], $fieldInfo[1], $fieldInfo[2], $fieldInfo[3], $fieldInfo[4]); 
        return $html;
    }
    ...
    {        
        ...
        if ($fieldProps["display"] == 'hidden' and is_array($fieldProps['value'])){
            foreach ($fieldProps['value'] as $id => $value){                
                $cureItems .= '<input type="hidden" name="'.$fieldProps["name"].'['.$id.']" value="'.$value.'" />';                
            }
            return $cureItems;
        }
    ...
    {        
        ...
        foreach ($fieldProps['value'] as $id){
            if ($fieldProps["display"] == 'hidden'){
                $cureItems .= '<input type="hidden" name="'.$fieldProps["name"].'[]" value="'.$id.'" />';
            } else {
                $name = $fieldProps["listValues"][$id][0] ?? '';
                $cureItems .= '<option value="'.$id.'" selected>'. $name.'</option>';
            }
        }
        ...
        if ($fieldProps['placeholder'] != '') $listItems = '<option value="">'. $fieldProps['placeholder'].'</option>';
        $fieldProps['value'] = (is_array($fieldProps['value'])) ? $fieldProps['value'] : [$fieldProps['value']];

        foreach ($fieldProps["listValues"] as $id => $info){
            $name = $info[0] ?? '';
            if (isset($fieldProps['value']) and in_array($id, $fieldProps['value'])) continue;
    ...
    protected function genFileField ($subType, $wraperProps, $fieldProps, $info=[])
    {
        $listItems = '';
        foreach ($fieldProps["listValues"] as $id => $info){
            if (!in_array($id, $fieldProps["value"])) continue;

            $name = $info[0] ?? '';
            $link = (isset($info[1]))? '/'.$info[1] : '#';
            ...
            <div class="ui clearing segment file-item" style="background: none;">
                ...
                '.$name.'
                <a title="Посмотреть файл" href="'.$link.'" target="_blank"><i class="external alternate icon"></i></a>
